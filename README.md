...
Схема сейчас стабильна, автоматический `prisma migrate deploy` в build-шаге
Vercel **намеренно не используется** — через пулер Neon (PgBouncer)
advisory-lock, которым пользуется Prisma Migrate, периодически подвисает и
роняет билд без реальной причины. Если меняете `prisma/schema.prisma`,
накатите миграцию вручную с прямым (unpooled) подключением:
