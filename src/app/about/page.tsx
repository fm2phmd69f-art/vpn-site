...
const SECTIONS = [
  {
    ...
    body: [
      ...
      "Сайт помогает быстро сравнить цены, платформы, заявленную скорость и особенности разных провайдеров в одном месте, а также подобрать сервис под конкретную задачу — например, для [Netflix](/vpn-for-netflix), [торрентов](/vpn/category/torrents) или анонимности.",
    ],
  },
  {
    ...
    body: [
      ...
      "Если провайдер проходил независимый внешний аудит политики no-logs (например, [NordVPN](/vpn/nordvpn), [ExpressVPN](/vpn/expressvpn) или [Surfshark](/vpn/surfshark)), это отдельно отмечается в описании сервиса — как факт публикации такого аудита, а не как наша собственная оценка.",
  ...
  },
  {
    ...
    body: [
      ...
      "Часть ссылок на сайты провайдеров может быть партнёрской (affiliate) — переход по такой ссылке и последующая оплата подписки может принести нам небольшое вознаграждение от провайдера. Это никак не влияет на цену для вас. При выборе сортировки по цене, скорости, рейтингу или VPNmarket Score порядок считается только по этим данным. В сортировке «Рекомендуем» (стоит по умолчанию) на 3-й позиции может показываться один оплаченный рекламный слот — такая карточка отдельно помечена меткой «Реклама».",
    ],
  },
  {
    ...
    body: [
      ...
      "Отзывы на страницах провайдеров можно оставить анонимно, без регистрации — мы не проверяем, что автор действительно пользовался сервисом, поэтому воспринимайте их как мнения, а не как подтверждённые покупки.",
